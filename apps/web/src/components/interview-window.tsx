...
export default function InterviewWindow({ scenarioId }: InterviewWindowProps) {
	...
	const sendMessageSubscription = trpc.interview.sendMessage.useSubscription(
		streamingInput,
		{
			...
			onData: (token) => {
				setMessages((prev) => {
					...
					if (
						lastMessage &&
						lastMessage.isAi &&
						lastMessage.id.startsWith("ai-streaming")
					) {
		...
		return (
			<main className="mx-auto max-w-4xl px-6 py-16">
				<Card className="p-6 text-center">
					<h2 className="mb-2 font-semibold text-xl">Ошибка</h2>
	...
	return (
		<main className="mx-auto max-w-4xl px-6 py-16">
			<div className="mb-6 flex items-center justify-between">
				<h1 className="font-semibold text-2xl">Интервью</h1>
				<Button
					...
					{messages.map((message) => (
						...
						>
							...
							>
								...
								{isStreaming &&
									message.isAi &&
									message.id.startsWith("ai-streaming") && (
